...
esp_err_t lcd_init(esp_lcd_panel_handle_t *panel_handle_out) {
    ...
    spi_bus_config_t buscfg = {
        .sclk_io_num = SMALLTV_PRO_LCD_SPI_SCLK_PIN,
        .mosi_io_num = SMALLTV_PRO_LCD_SPI_MOSI_PIN,
        .miso_io_num = -1,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
    ...
    ESP_ERROR_CHECK(spi_bus_initialize(SMALLTV_PRO_LCD_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO));

    ESP_LOGI(TAG, "Attach LCD panel to SPI bus");
    esp_lcd_panel_io_handle_t io_handle = NULL;
    esp_lcd_panel_io_spi_config_t io_config = {
        .dc_gpio_num = SMALLTV_PRO_LCD_SPI_DC_PIN,
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)SMALLTV_PRO_LCD_SPI_HOST,
                                             &io_config, &io_handle));

    ESP_LOGI(TAG, "Create St7789 LCD panel");
    esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = SMALLTV_PRO_LCD_RST_PIN,
...
esp_err_t backlight_onoff(bool enable) {
    ...
    if (enable) {
        ...
        ESP_ERROR_CHECK(gpio_set_level(SMALLTV_PRO_LCD_BL_PIN, SMALLTV_PRO_LCD_BL_OFF_LEVEL));
    }
