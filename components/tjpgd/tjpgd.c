    ...
    do {
        if (!bm) {              /* Next byte? */
            ...
            if (flg) {   /* In flag sequence? */
                ...
                if (*dp != 0)
                    return 0 - (int)JDR_FMT1; /* Err: unexpected flag is detected (may be collapted
                                                 data) */
...
JRESULT jd_prepare(JDEC *jd,                                    /* Blank decompressor object */
    ...
    memset(jd, 0, sizeof(JDEC)); /* Clear decompression object (this might be a problem if machine's
    ...
    for (;;) { /* Parse JPEG segments */
        ...
        switch (marker & 0xFF) {
            ...
            case 0xDA: /* SOS - Start of Scan */
                ...
                jd->workbuf = alloc_pool(jd, len); /* and it may occupy a part of following MCU
                                                      working buffer for RGB output */
                if (!jd->workbuf) return JDR_MEM1; /* Err: not enough memory */
                jd->mcubuf = alloc_pool(
                    jd, (n + 2) * 64 * sizeof(jd_yuv_t)); /* Allocate MCU working buffer */
